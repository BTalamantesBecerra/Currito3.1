...
def main(argv):
    ...
    print ('Circus program PATH for etc = ', circos_program_path)

    inputFileToArrange = open(InputBLASTfiltered)
    OutputFile = open(Output_preparingBLAST_data_path, 'a')

    highlights_sequences = output_preCIRCOS_path + "highlights_sequences.txt"
    pident = output_preCIRCOS_path + "pident.txt"
    subjectAcc = ""

    outputHighLights = open(highlights_sequences, 'a')
    outputPident = open(pident, 'a')

    karyotype = output_preCIRCOS_path + "karyotype.txt"
    outputkaryotype = open(karyotype, 'a')
    sLength = ""
    ImageLabelNumber = ""
    ...
    outputHighL = open(highlights,'a')


    ticks = output_preCIRCOS_path + "ticks.txt"
    outputTicks = open(ticks, 'a')



    conf = output_preCIRCOS_path + "conf.txt"
    outputConf = open(conf,'a')
    ...
    "<spacing>" + '\n' + "default = 0u" + '\n' + "break = 0u" + '\n' +
    "</spacing>" + '\n' + "thickness = 20p" + '\n' +
    "radius = 0.70r" + '\n' + "show_label = yes" + '\n' + "label_font = bold" + '\n' + "label_with_tag = yes" + '\n' + "label_radius = dims(ideogram,radius) + 0.28r" + '\n' +
    "label_size = 30" + '\n' + "label_parallel   = yes" + '\n' + "stroke_thickness = 3" + '\n' + "stroke_color = gray" + '\n' + "fill = yes" + '\n' + "show_bands = yes" + '\n' +
    "fill_bands = yes" + '\n' +
    "</ideogram>" + '\n' +
    "<<include " + ticks + ">>" + '\n' +
